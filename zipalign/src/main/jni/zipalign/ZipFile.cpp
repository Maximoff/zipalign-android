...
status_t ZipFile::open(const char *zipFileName, int flags) {
    ...
    assert(mZipFp == nullptr);     // no reopen
...
status_t ZipFile::addCommon(const char *fileName, const void *data, size_t size,
    ...
    assert(mZipFp != nullptr);
    assert(mEntries.size() == mEOCD.mTotalNumEntries);

    /* make sure it doesn't already exist */
    if (getEntryByName(storageName) != nullptr)
        return ALREADY_EXISTS;

    if (!data) {
        inputFp = fopen(fileName, FILE_OPEN_RO "e");
        if (inputFp == nullptr)
    ...
    pEntry = new ZipEntry;
    pEntry->initNew(storageName, nullptr);
    ...
    if (sourceType == ZipEntry::kCompressStored) {
        ...
        result = copyPartialFpToFp(mZipFp, inputFp, compressedLen, nullptr);
        if (result != NO_ERROR) {
